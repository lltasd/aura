// @ts-nocheck
/*
  Vercel serverless function: /api/email/verify
  Validates code against token produced by /api/email/send
...
export default async function handler(req: any, res: any) {
  ...
  const { email, exp, salt } = payload || {}
  if (!email || !exp || !salt) return json(res, 400, { ok: false, error: 'Bad token payload' })

  const now = Math.floor(Date.now() / 1000)
  if (now > Number(exp)) return json(res, 400, { ok: false, error: 'Code expired' })

  const expected = sign(secret, `${String(code)}|${String(email)}|${String(exp)}|${String(salt)}`)
